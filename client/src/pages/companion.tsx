...
export default function CompanionApp() {
  ...
  const lookupMutation = useMutation({
    mutationFn: async (fullPhoneNumber: string) => {
      ...
      const response = await apiRequest(`/api/devices/lookup/mobile/${encodeURIComponent(fullPhoneNumber)}`);
      return response;
    },
    onSuccess: async (data: LookupResult) => {
      ...
      // Check if either identifier matches
      ...
      const validationMethod = phoneMatches && imeiMatches ? 'both identifiers' : phoneMatches ? 'phone number' : 'device IMEI';
      addDebugLog(`Device validated successfully using ${validationMethod}`);
      
      toast({
        title: "Device Validated",
        description: `Connection approved using ${validationMethod}`,
      });
      
      setLookupResult(data);
      
      // Auto-populate IMEI field with real IMEI from database
      setDeviceIMEI(data.imei);
      
      // Save IMEI to localStorage for persistence
    ...
    onError: (error: Error) => {
      ...
      toast({
        ...
        description: "This phone number is not registered in the parent dashboard. Please ask your parent to add your device first.",
        variant: "destructive",
      });
    },
  });
  ...
  const handleLookup = () => {
    ...
    const fullPhoneNumber = cleanPhoneNumber.startsWith('91') ? cleanPhoneNumber : `${countryCode.replace('+', '')}${cleanPhoneNumber}`;
    
    addDebugLog(`Trying phone formats: +91${cleanPhoneNumber}, +91 ${cleanPhoneNumber}, ${fullPhoneNumber}`);
    lookupMutation.mutate(fullPhoneNumber);
  };
  ...
  const consentMutation = useMutation({
    mutationFn: async ({ imei, action }: { imei: string; action: 'approve' | 'deny' }) => {
      ...
      const response = await apiRequest(`/api/companion/consent/${encodeURIComponent(imei)}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    if (phoneNumber.trim() && phoneNumber.length >= 10) {
      // Try multiple phone number formats for better compatibility
      const formats = [
        `${countryCode}${phoneNumber}`,        // +918870929411
        `${countryCode} ${phoneNumber}`,       // +91 8870929411  
        phoneNumber                            // 8870929411
      ];
      
      // Try the first format
      addDebugLog(`Trying phone formats: ${formats.join(', ')}`);
      lookupMutation.mutate(formats[1]); // Use the format with space first
    }
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-600 mb-1">
                  Enter the phone number your parent registered for this device
                </p>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Enter the mobile number your parent registered. We'll automatically fetch your device details and IMEI.
                </p>
              </div>
              ...
              {!lookupResult && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <h4 className="text-sm font-medium text-blue-800 mb-2">📱 How Device Connection Works:</h4>
                  <div className="text-xs text-blue-700 space-y-1">
                    <div>1. Enter your phone number above</div>
                    <div>2. We'll find your device in parent's database</div>
                    <div>3. Your IMEI will be automatically retrieved</div>
                    <div>4. Device connects instantly - no manual entry needed!</div>
                  </div>
                </div>
              )}
              ...
              {!lookupResult ? (
                <Button 
                  ...
                  disabled={!phoneNumber.trim() || lookupMutation.isPending}
                >
                  {lookupMutation.isPending ? "Searching Database..." : "Connect My Device"}
                </Button>
              ) : (
                <Button 
                  onClick={registerDevice}
                  className="w-full bg-green-600 hover:bg-green-700"
                  disabled={!deviceIMEI.trim()}
                >
                  Register with Knets
                </Button>
              )}
